...
# اختيار الشخصية
persona = st.radio("اختر شخصية المساعد:", ["رسمية سياسية", "تحليل علم نفس", "تعليم طفل"])
...
bnb_config = BitsAndBytesConfig(
    load_in_8bit=True,  # أو load_in_4bit لو عايزة ضغط أعلى
...
model = AutoModelForCausalLM.from_pretrained(
    model_name,
    quantization_config=bnb_config,  # هنا بدل load_in_8bit
    device_map="auto"
)
...
# الإجابة المتوقعة (ground truth)
reference = ["الديمقراطية هي أن الناس يختاروا من يمثلهم بطريقة عادلة."]

# الإجابة اللي طلعها الموديل
prediction = ["الديمقراطية معناها إن الشعب يختار ممثليه بشكل عادل."]
